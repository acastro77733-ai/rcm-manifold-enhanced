...
class NonlinearFieldModel(FieldModel):
    def step(self, node_summary, mean_energy, mean_confidence, topology_stats):
        ...
        nonlinear = np.tanh(1.2 * drive) + 0.05 * np.sign(drive)
        diffusion = 0.12 * np.clip(self.field_state - nonlinear, -1.0, 1.0)
        noise = self.rng.normal(0.0, 0.01, size=self.field_state.shape[0])
        self.field_state = np.clip(0.72 * self.field_state + 0.28 * nonlinear + diffusion + noise, -2.0, 2.0)

        node_modulation = np.tanh(self.field_state)
        field_variance = float(np.var(self.field_state))
        synchronization_pressure = float(
            np.clip(0.5 * mean_confidence + 0.3 * topology_stats.get("stability", 0.0) + 0.2 * topology_stats.get("density", 0.0), 0.0, 1.0)
        )
        plasticity_pressure = float(
            np.clip(0.6 * field_variance + 0.2 * (1.0 - mean_confidence) + 0.2 * topology_stats.get("density", 0.0), 0.0, 1.0)
        )
